...
class YXG_KCMBXG_ALL_BINS_Likelihood(GaussianLikelihood):
    ...
    def initialize(self):
        ...
        for i in range(1, Nbins+1):
            # print("Bin", i)
            D_kg = np.loadtxt(os.path.join(self.data_directory)+  f'kcmbg_cardinal_2048_DES_Maglim_{i}_dl.txt')
            D_yg = np.loadtxt(os.path.join(self.data_directory)+  f"yg_cardinal_4096_DES_Maglim_{i}_B12_dl.txt")
        ...
        # print("cl data:",self.cl_joint)
        super().initialize()


    def get_requirements(self):
        return {"Cl_galnxtsz": {}, "cl_galn_lens": {}}
    ...
    def _get_theory(self, **params_values_dict):
        ...
        theory_kg = self.provider.get_cl_galn_lens()
        for i in range(len(theory_yg)):
            ...
            ell_kg_bin, dl_kg_bin_1h = self._bin(ell_theory_kg, np.asarray(cl_1h_theory_kg), self.ell_kg, ellmax_bin_kg, bpwf_kg,  Nellbins=Np_kg, conv2cl=True)
            ell_kg_bin, dl_kg_bin_2h = self._bin(ell_theory_kg, np.asarray(cl_2h_theory_kg), self.ell_kg, ellmax_bin_kg, bpwf_kg,  Nellbins=Np_kg, conv2cl=True)
        ...
        # print("yg: ", np.concatenate(yg_all))
        # print("kg: ", np.concatenate(kg_all))
