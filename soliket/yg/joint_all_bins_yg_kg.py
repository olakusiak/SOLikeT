...
class YXG_KXG_ALLBINS_Likelihood(GaussianLikelihood):
    ...
    def initialize(self):
        ...
        Np_yg = self.Nbins_yg
        Np_kg = self.Nbins_kg
        # yg_ACT_DR6_healpy_4096_ilc_SZ_deproj_cib_dBeta_1.77_10.14_yy_DES_Maglim_1_dl.txt
        Nbins = 4
    ...
    def _get_theory(self, **params_values):
        ...
        for i in range(len(theory_yg)):
            ...
            ell_km_bin, dl_km_bin = self._bin(ell_theory_km, np.asarray(cl_1h_theory_kg) + np.asarray(cl_2h_theory_kg), self.ell_kg_full, ellmax_bin_kg, bpwf_kg, pixwin_kg, Nellbins=Np_kg, conv2cl=True)
            ...
            kg = dl_kg_bin + 2*(alpha-1)*dl_km_bin + dl_gIA_bin
