...
class CMBKXG_ALLBINS_Likelihood(GaussianLikelihood):
    ...
    def initialize(self):
        ...
        Np_yg = 21*4
        self.covmat =  self.covmat[:Np_kg*4,:Np_kg*4]
        ...
        self.cl_joint = np.concatenate((Cl_kg_all), axis=0)
        self.ell_joint = np.concatenate((self.ell_kg, self.ell_kg,self.ell_kg, self.ell_kg), axis=0)
        # print("self.ell_joint:", self.ell_joint)
        # print("self.cl_joint:", self.cl_joint)
        super().initialize()

    def get_requirements(self):
        return {"cl_galn_lens": {}, "Cl_lensmagnxlens": {}}
    ...
    def _get_theory(self, **params_values):
        ...
        for i in range(len(theory_kg)):
            ...
            kg = dl_kg_bin + 2*(alpha-1)*dl_km_bin
            kg_all.append(kg)
            # print("kg: ", kg_all)

        cl_joint = np.concatenate((kg_all), axis=0)
