...
class GXG_Likelihood(GaussianLikelihood):
    ...
    #tranfer_funct_file: Optional[str] = None
    ...
    def initialize(self):
        ...
        self.bpwf = np.loadtxt(os.path.join(self.data_directory, self.bp_wind_file))

        #load data
        D = np.loadtxt(os.path.join(self.data_directory, self.gxg_data_file))
        self.ell = D[0,1:]
        self.gg = D[1,1:]
        self.sigma = D[2,1:]
        self.ell_full = D[0,]
        ...
        self.covmat = np.asarray(self.sigma_tot)**2.
    ...
    def _get_theory(self, **params_values):
        ...
        ell_gm_binned, cl_gm_binned = self._bin(ell_theory, dl_gm_theory, self.ell, self.bpwf, Nellbins=25)
