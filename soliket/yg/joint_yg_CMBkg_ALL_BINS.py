...
class YXG_CMBKXG_ALLBINS_MISCENTER_Likelihood(GaussianLikelihood):
    data_directory: Optional[str] = None
    y_map: Optional[str] = None
    params = {'cmis0': 0, 'cmis1': 0, 'cmis2': 0,'cmis3': 0}
    ...
    def initialize(self):
        ...
        for i in range(1, Nbins+1):
            ...
            D_kg = np.loadtxt(self.data_directory + self.gxk_data_file + str(int(i)) + "_kappabaseline_dl.txt")
    ...
    def _get_theory(self, **params_values_dict):
        ...
        for i in range(4):
            ...
            print("yg 2h =", yg_2h_all[i])
            print("kg 1h =", kg_1h_cen_mis+kg_1h_sat)
            print("kg 2h =", kg_2h_all[i])
            yg = 1.e-6*( yg_1h_cen_mis+yg_1h_sat +yg_2h_all[i]+ 2*(alpha-1)*ym_all[i] )
            kg = (kg_1h_cen_mis+kg_1h_sat +kg_2h_all[i] + 2*(alpha-1)*km_all[i]) 
            yg_all.append(yg)
            kg_all.append(kg)
        print("kg: ", kg_all)
        print("yg: ", yg_all)
