...
:running ola nersc: cobaya-run ~/software/Likelihoods_sz/software/Likelihoods_sz/soliket/ymap/input_files/red_gxg_ola_cori_mcmc.yaml -f
...
class joint_KXG_GXG_PS_Likelihood(GaussianLikelihood):
    ...
    wind_funct_file_kg: Optional[str] = None
    tranfer_funct_file_gg: Optional[str] = None
    tranfer_funct_file_kg: Optional[str] = None
    #params = {"lA": 0}
    ...
    def initialize(self):
        ...
        self.wind_kg = np.loadtxt(os.path.join(self.data_directory, self.wind_funct_file_kg))
        #transfer functions
        self.trans_gg = np.loadtxt(os.path.join(self.data_directory, self.tranfer_funct_file_gg))
        self.trans_kg = np.loadtxt(os.path.join(self.data_directory, self.tranfer_funct_file_kg))
        # print("tranfer gg:")
        # print(self.trans_gg)
        # print("tranfer kg:")
        # print(self.trans_kg)
    ...
    def _get_theory(self, **params_values):
        s=self.s
        A_shotnoise = params_values['A_shot_noise']
        trans_gg = self.trans_gg
        trans_kg = self.trans_kg
        shot_noise  = A_shotnoise * 1e-7
        ...
        ell_gg_binned, cl_gg_binned = self.binning(ell, dl_gg_theory, self.ell_full, self.wind_gg, Nellbins=9, ellmin = 100.5, ellmax = 1000.5)
        ...
        ell_gm_binned, cl_gm_binned = self.binning(ell, dl_gm_theory, self.ell_full, self.wind_gg, Nellbins=9, ellmin = 100.5, ellmax = 1000.5)
        ...
        ell_mm_binned, cl_mm_binned = self.binning(ell, dl_mm_theory, self.ell_full, self.wind_gg, Nellbins=9, ellmin = 100.5, ellmax = 1000.5)
        ...
        ell_kg_binned, cl_kg_binned = self.binning(ell_theory_kg, dl_kg_theory, self.ell_full, self.wind_kg, Nellbins=9, ellmin = 100.5, ellmax = 1000.5)
        ...
        ell_km_binned, cl_km_binned = self.binning(ell_theory, dl_km_theory, self.ell_full, self.wind_kg, Nellbins=9, ellmin = 100.5, ellmax = 1000.5)
        # print('cl kg: ', cl_kg_binned)
        # print('cl km: ', cl_km_binned)
        # print('cl kg: ', cl_kg_binned + (5*s-2)*cl_km_binned)

        kg = cl_kg_binned + (5*s-2)*cl_km_binned
        gg = cl_gg_binned + 2*(5*s-2)*cl_gm_binned + (5*s-2)*(5*s-2)*cl_mm_binned + shot_noise #1e-7*params_values['A_shot_noise']
